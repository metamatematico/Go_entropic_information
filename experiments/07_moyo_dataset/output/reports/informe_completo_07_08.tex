...
\subsection{Pr\'oximos pasos, ordenados}

\begin{enumerate}
  \item \good{Hecho}: Fase~B (Secci\'on~\ref{sec:faseb}) encontr\'o
        $H_{OPT\_B}$, con $\Delta R^2=0.421$ sobre las 20 partidas e
        IC~95\,\% $[0.339,0.486]$ --- por encima, con traslape angosto,
        de $H_{M1}$ y $H_{OPT\_A}$. Es la evidencia m\'as fuerte del
        experimento de que ampliar a la familia completa
        \texttt{cubic\_mixed} (con t\'erminos $b$) ayuda. Queda
        pendiente resolver la advertencia metodol\'ogica sobre
        $b_{12}$ en el borde de su rango antes de adoptarlo como
        candidato definitivo.
  \item Decidir sobre esa advertencia: ¿ampliar el rango permitido de
        $b_{12}$ (perdiendo comparabilidad directa con el cat\'alogo de
        47) o mantenerlo (aceptando que $H_{OPT\_B}$ es el mejor
        \emph{dentro} de la familia estudiada)? Si se ampl\'ia,
        repetir Fase~B con el nuevo rango.
  \item Conseguir m\'as partidas (o separar un conjunto de validaci\'on)
        --- ahora principalmente para confirmar la ventaja de
        $H_{OPT\_B}$ con un traslape a\'un menor, y para resolver el
        empate persistente entre $H_{M1}$ y $H_{OPT\_A}$
        (Secci\'on~\ref{sec:bootstrap}).
  \item Integrar al visor interactivo los 25 candidatos
        \texttt{sparse\_cubic} generados al azar (Secci\'on~3.2) y
        $H_{OPT\_B}$, pendiente desde el inicio del experimento.
  \item Categor\'ias 6 (grupos de piedras / vida-muerte) y 7 (frontera
        activa / aji) de la Secci\'on~\ref{sec:categorias}: a diferencia
        de joseki/fuseki/sente-gote, requieren un mecanismo
        \textbf{nuevo} (agregaci\'on a nivel de grupo de piedras, no a
        nivel de punto) --- no reutilizan \texttt{relaxation\_field}
        directamente.
\end{enumerate}
